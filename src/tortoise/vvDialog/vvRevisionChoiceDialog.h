 ...
 * a wxTextEntryDialog.
 */
class vvRevisionChoiceDialog : public vvDialog
{
// functionality
public:
	vvRevisionChoiceDialog::vvRevisionChoiceDialog()
	{
	...
	virtual bool Create(
		...
		const wxString&		sTitle,							  //< [in] The title of the dialog.
		const wxString&		sDescription,					  //< [in] The description to use on the dialog.
...
private:
	// internal data
	vvHistoryListControl * m_historyControl;
	vvVerbs::HistoryData m_data;
...
// macro declarations
private:
	DECLARE_DYNAMIC_CLASS(vvRevisionChoiceDialog);
	DECLARE_EVENT_TABLE();
};
