...
(function ()
{
    ...
    Markdown.Editor = function (markdownConverter, idPostfix, help)
    {
        ...
        var that = this,
            panels;
        this.setVvMarkdown = function (md)
        {
            util.vvMarkdown = md;
            util.idPostfix = idPostfix;
        }
        this.run = function ()
        {
            if (panels)
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions)
    {
        ...
        function makeSpritedButtonRow()
        {
            ...
            makeSpacer(4);
            buttons.addBug = makeButton("wmd-bug-button", "Insert Bug Link - ^", "-260px", null);
            buttons.addBug.execute = function (manager)
            {
                if (util.vvMarkdown && util.vvMarkdown.promptForBug)
                {
                    util.vvMarkdown.promptForBug();
                }
            };

            buttons.addCs = makeButton("wmd-cs-button", "Insert Changeset Link - @", "-280px", null);
            buttons.addCs.execute = function (manager)
            {
                if (util.vvMarkdown && util.vvMarkdown.promptForCs)
                {
                    util.vvMarkdown.promptForCs();
                }
            };

            buttons.addFilter = makeButton('wmd-filter-button', "Insert Work Item Filter Link", "-300px", null);
            buttons.addFilter.execute = function (manager)
            {
                if (util.vvMarkdown && util.vvMarkdown.promptForFilter)
                {
                    util.vvMarkdown.promptForFilter();
                }
            };
			
            buttons.addWiki = makeButton('wmd-wiki-button', "Insert Wiki Link", "-320px", null);
            buttons.addWiki.execute = function (manager)
            {
                if (util.vvMarkdown && util.vvMarkdown.promptForWiki)
                {
                    util.vvMarkdown.promptForWiki();
                }
            };
