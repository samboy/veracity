...
function st_named_branches_08()
{
    this.no_setup = true;

    this.test_8 = function test_8() 
    {
        var r1_name = sg.gid();
...
// confirm update set branch
		...
		//LOOK OUT!// This is expected to fail, since foo has multiple heads
        o = sg.exec(vv, "update", "-b", "foo");
        print(sg.to_json(o));
        testlib.ok(o.exit_status != 0);

        // -------- remove foo
        o = sg.exec(vv, "branch", "remove-head", "foo", "--all");
        print(sg.to_json(o));
        testlib.ok(o.exit_status == 0);
