...
function st_locks_01()
{
	...
	vscript_test_wc__lock_np( { "src" : "f1.txt" },
        ...
        // -------- in r3, check for the lock
        sg.fs.cd(r3_path);

	sg.vv2.whoami({ "username" : "paul" });

        // without pulling the new locks info, the lock isn't there
        ...
        // -------- in r4, try to unlock it as the wrong user
        sg.fs.cd(r4_path);
	...
	// use internal JS version of unlock and expect it to fail.
	vscript_test_wc__unlock_np( { "src" : "f1.txt" },
				    false );

        // now use --force to do it
	// use internal JS version of unlock and expect it to succeed.
	vscript_test_wc__unlock_np( { "src" : "f1.txt",
				      "force" : true },
				    true );
    }
